...
public class starEdit extends JPanel {
    public starEdit() {
        super(new GridLayout(2, 2, 4, 4));

    }

    public JPanel makeStarRatingPanel(String title, final LevelBarEdit label) {
    ...
    public static ImageIcon makeStarImageIcon(ImageProducer ip, float rf, float gf, float bf) {
        return new ImageIcon(Toolkit.getDefaultToolkit().createImage(new FilteredImageSource(ip, new SelectedImageFilterEdit(rf, gf, bf))));
    }

}

class LevelBarEdit extends JPanel implements MouseListener, MouseMotionListener {
    ...
    protected LevelBarEdit(ImageIcon defaultIcon, List<ImageIcon> list, int gap) {
...
class SelectedImageFilterEdit extends RGBImageFilter {
    private final float rf;
    private final float gf;
    private final float bf;

    protected SelectedImageFilterEdit(float rf, float gf, float bf) {
        super();
